...
class _DocumentosEventoState extends State<DocumentosEvento> {
                              ...
                              switch (value) {
                                case 1:
                                  viewPdf(documentos[index].nombreDoc,
                                      base64Decode(documentos[index].codImg!));
                                  break;
                                case 2:
  ...
  void deleteDoc(int id) async {
    ...
    if (res == 1) {
      ...
      scaffoldMessenger(context, "Documento eliminado",
          icon: const Icon(Icons.check, color: Colors.green));
    } else {
      scaffoldMessenger(context, "No se elimino el documento",
  ...
  void seleccionarArchivo() async {
    ...
    if (file != null) {
      ...
      if (res != 0) {
        await obtenerDocumentos();
        scaffoldMessenger(context, "Documento agregado",
            icon: const Icon(Icons.check, color: Colors.green));
      } else {
        scaffoldMessenger(context, "No se agrego el documento",
            icon: const Icon(Icons.error, color: Colors.red));
      }
    } else {
      scaffoldMessenger(context, "Documento no seleccionado",
